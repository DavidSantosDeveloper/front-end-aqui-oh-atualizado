...
const Detalhamento: React.FC <DetalhamentoProps>= () => {
    ...
    async function atualizarStatusReclamacao(status_atual: string) {
        ...
        try {
            ...
            const response = await fetch(`https://back-end-aqui-oh-atualizado-production.up.railway.app:8080/api/reclamacao/${id}`, {
                method: 'PUT',
                headers: {
    ...
    useEffect(() => {
        const fetchMensagens = async () => {
            try {
                const token_usuario = localStorage.getItem("jwt_access");
                const response = await axios.get('https://back-end-aqui-oh-atualizado-production.up.railway.app:8080/api/mensagem/'+id, {
                    headers: {
                        Authorization: `Bearer ${token_usuario}`,
